...
if __name__ == '__main__':
    ...
    # Monitoring settings
    parser.add_argument("-c", "--cumulative", dest='cumulative',
                        action='store_true',
                        help="Always display all the hits,"
                             " not only the most recent")
    # Alerting system
    parser.add_argument("--alert-threshold", default=50,
                        help="Number of hits within the alert delay"
                        " that will raise an alert")
    parser.add_argument("--alert-delay", default=2,
                        help="Number of minutes we need to"
                        " watch the traffic for")
